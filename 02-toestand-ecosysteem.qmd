---
lightbox: auto
description: "Rapportage waterkwaliteit van waterschap Hoogheemraadschap van Schieland en de Krimpenerwaard (HHSK) 2025. Dit hoofdstuk gaat over de algemene waterkwaliteitstoestand. Het gaat over de hoeveelheid waterplanten en kroos; over de invasieve exoot de Amerikaanse Rivierkreeft en over de aanwezigheid van blauwalgen"
---

# Toestand ecosysteem {#sec-toestand}

```{r code-toestand-planten, cache = FALSE}
#| file: "R/CODE_toestand-ecosysteem.R"
#| include: false
```

::: {.samenvatting}

Er komen steeds meer Amerikaanse rivierkreeften. Het aantal is in de laatste vijf jaar verdubbeld. Met de opkomst van de kreeften verdwijnen de waterplanten en zelfs kroos steeds meer uit het gebied. De afwezigheid van planten leidt ook tot steeds meer blauwalgen.

:::

------------------------------------------------------------------------

### Inleiding {.unnumbered}

::: aside
![](images/vince_sad2.svg){.nolightbox fig-align="center" width="200" fig-alt="Pictogram van een verdrietig kijkende vis"}
:::

Planten en dieren kunnen een grote invloed uitoefenen op het ecosysteem. Planten die onder water groeien hebben een belangrijke positieve bijdrage. Kroos, rivierkreeften en blauwalgen kunnen een grote negatieve invloed hebben op het ecosysteem. In dit hoofdstuk wordt de toestand van het ecosysteem beschreven aan de hand van de genoemde groepen.

### Waterplanten {.unnumbered}

Waterplanten hebben een sleutelrol in het onderwaterleven (ecosysteem). Waterplanten zorgen voor de productie van zuurstof en vormen het voedsel en leefgebied voor vissen en andere waterdieren. Door deze sleutelrol geven waterplanten een goede indicatie van de waterkwaliteit en de toestand van het leven onder water. De meeste planten hebben een positieve rol in het ecosysteem. Er zijn echter ook soorten, zoals kroos, die kunnen zorgen voor problemen.

In de Krimpenerwaard is het aantal locaties met planten onder water[^02-toestand-ecosysteem-1] sinds 2017 sterk afgenomen (@fig-submers). [**In 2025 werd op geen enkel meetpunt in de Krimpenerwaard nog enige bedekking met onderwaterplanten aangetroffen.**]{.donkerblauw} Ook in Schieland neemt het aantal locaties met planten onder water steeds verder af. In Schieland is de afname echter kleiner en minder snel dan in de Krimpenerwaard. Het is aannemelijk dat de afname van waterplanten komt door de opkomst van Amerikaanse rivierkreeften.

[^02-toestand-ecosysteem-1]: Als grens voor wateren met waterplanten is aangehouden dat meer dan 5% van de oppervlakte met waterplanten begroeid moet zijn.

```{r}
#| label: fig-submers
#| out-width: "100%"
#| fig-cap: "De ontwikkeling van het aantal wateren met planten onder water in de Krimpenerwaard en in Schieland."
#| fig-alt: "Lijngrafiek met de ontwikkeling van het aantal wateren met planten onder water in de Krimpenerwaard en in Schieland."
#| lightbox:
#|   group: toestand
plot_submers
```

::: {.callout-note collapse="true"}
## Hoe zit het met kroos?

Kroos[^02-toestand-ecosysteem-2] kan hard groeien als er (te) veel voedingsstoffen in het water aanwezig zijn. Kroos drijft op het water en kan grote delen van het water afdekken. Hierdoor is er onvoldoende licht voor andere waterplanten en is er minder zuurstofuitwisseling met de lucht. Als een groot gedeelte van het water met kroos is begroeid heeft dat een negatieve invloed op het water leven.

Het aantal wateren met veel kroos is te zien in onderstaande figuur. Het aantal wateren met veel kroos varieert sterk van jaar tot jaar. In de Krimpenerwaard zijn er soms veel meer wateren met veel kroos dan in Schieland. In de laatste jaren is het aantal wateren met veel kroos in de Krimpenerwaard juist opvallend laag. De afwezigheid van kroos is positief, maar wordt in de Krimpenerwaard waarschijnlijk veroorzaakt door de grote hoeveelheid kreeften.

```{r}
#| label: fig-kroos
#| out-width: "100%"
#| fig-cap: "De ontwikkeling van het aantal wateren met veel kroos in de Krimpenerwaard en in Schieland."
#| fig-alt: "Lijngrafiek met de ontwikkeling van het aantal wateren met veel kroos in de Krimpenerwaard en in Schieland."
#| lightbox:
#|   group: toestand
plot_kroos
```
:::

[^02-toestand-ecosysteem-2]: Kroos bestaat uit kleine plantjes die los op het water drijven.

{{< pagebreak >}}

### Kreeften {.unnumbered}

Amerikaanse rivierkreeften[^02-toestand-ecosysteem-3] hebben een negatieve invloed op het watersysteem. Ze graven in de oevers waardoor oevers afkalven en er meer bagger in de watergangen ontstaat. Ook zijn de kreeften echte alleseters: veel planten en dieren die van nature voorkomen worden door de kreeften opgegeten. De kreeften zijn waarschijnlijk de oorzaak van de afname van waterplanten in het gebied. De massale aanwezigheid van de kreeften is een groot probleem voor de waterkwaliteit.

[^02-toestand-ecosysteem-3]: Het gaat bijna altijd over de Rode amerikaanse rivierkreeft (*Procambarus clarkii*). Dit is de soort die bijna overal en in de hoogste aantallen wordt aangetroffen. Er komen nog drie andere soorten voor het in het gebied. Deze soorten komen echter op weinig plekken voor en de hoeveelheid is altijd klein.

Het waterschap doet sinds 2020 jaarlijks onderzoek naar de aanwezigheid van Amerikaanse rivierkreeften.[^02-toestand-ecosysteem-4] Hierdoor weten we dat de kreeften overal in de Krimpenerwaard aanwezig zijn. In Schieland zijn niet overal kreeften aanwezig. Maar het aantal locaties neemt daar wel sterk toe. Vijf jaar geleden waren kreeften aanwezig op ongeveer 30% van de locaties. De laatste jaren is dit echter verdubbeld en in 2025 kwamen er zelfs op bijna 80% van alle locaties kreeften voor ([-@fig-kreeften-locaties]). Alleen aan de westkant van Schieland zijn nog enkele locaties waar geen kreeften zijn aangetroffen ([-@fig-kaart-kreeften]).

```{r}
#| label: fig-kreeften-locaties
#| fig-cap: Het gedeelte van alle onderzoekslocaties waar kreeften zijn aangetroffen.
#| fig-alt: Grafiek met het gedeelte van alle onderzoekslocaties waar kreeften zijn aangetroffen.
#| lightbox:
#|   group: toestand
plot_kreeften_aandeel_locs
```

```{r}
#| label: fig-kaart-kreeften
#| out-width: "100%"
#| fig-height: 3.5
#| fig-cap: "Op welke locaties zijn in {{< var rap_jaar >}} kreeften aangetroffen? (Interactief)"
#| fig-alt: "Kaart met de locaties waar in {{< var rap_jaar >}} kreeften zijn aangetroffen."
kaart_kreeften
```

[^02-toestand-ecosysteem-4]: In de [rapportage over Amerikaanse Rivierkreeften](https://www.schielandendekrimpenerwaard.nl/kaart/waterkwaliteit/waterkwaliteitsrapportages/kreeften_2025/){target="_blank"} is meer te lezen over de onderzoeksmethode en de resultaten van het onderzoek in {{< var rap_jaar >}}.

Er is niet alleen een toename in het aantal plekken waar kreeften voorkomen. Er is ook een opvallende toename in het aantal kreeften dat bij het onderzoek gevangen wordt. [**Zowel in de Krimpenerwaard als in Schieland is het aantal gemiddeld gevangen kreeften in vijf jaar verdubbeld.**]{.donkerblauw} ([-@fig-kreeften-aantallen]). Er is wel duidelijk een verschil tussen het aantal kreeften in de Krimpenerwaard en in Schieland: in de Krimpenerwaard zijn de aantallen veel groter.

```{r}
#| label: fig-kreeften-aantallen
#| fig-cap: Het gemiddeld aantal gevangen kreeften. Er is een getrimd gemiddelde gebruikt waarbij de meest extreme 5% van de waarden niet gebruikt zijn voor het bepalen van het gemiddeld aantal gevangen kreeften.
#| fig-alt: Grafiek met het gemiddeld aantal gevangen kreeften.
#| lightbox:
#|   group: toestand
plot_kreeften_aantallen
```

Het onderzoek naar de kreeften laat zien dat de aanwezigheid van de kreeften een weidverbreid probleem is. Het probleem lijkt in de afgelopen vijf jaar nog sterk te zijn toegenomen. Dit blijkt zowel uit het verdwijnen van de waterplanten als de verdubbeling van het aantal rivierkreeften.

{{< pagebreak >}}

### Blauwalgen {.unnumbered}

Blauwalgen zijn vaak te herkennen aan een intense groene kleur van het water.[^02-toestand-ecosysteem-5] Blauwalgen zijn geen echte algen maar een bepaalde groep bacteriën (cyanobacteriën) die zich gedragen als algen. Onder voedselrijke omstandigheden kunnen blauwalgen massaal gaan groeien. Hierdoor wordt het water troebel waardoor andere planten niet kunnen groeien. Blauwalgen vormen ook vaak drijflagen die na verloop van tijd afsterven en gaan rotten. Zo'n afstervende drijflaag gaat behoorlijk stinken. Drijflagen van blauwalgen zijn makkelijk te herkennen aan hoe ze eruit zien en soms ook aan de stank. De aanwezigheid van veel blauwalgen is een aanwijzing dat de waterkwaliteit niet zo goed is.

[^02-toestand-ecosysteem-5]: Ondanks hun naam zijn blauwalgen niet blauw, maar groen. De naam blauwalgen hebben ze gekregen door de blauwe kleurstof die vrijkomt als ze afsterven.

In de laatste jaren waren er in de sloten van de Krimpenerwaard veel plekken met veel blauwalgen voor. Dat is vrij ongebruikelijk voor sloten. Dit komt waarschijnlijk doordat er vrijwel geen concurrerende onderwaterplanten en kroos meer zijn. Toch waren er in {{< var rap_jaar >}} minder plekken met heel veel blauwalgen dan in eerdere jaren ([-@fig-kaart-blauwalgen]). 

In @sec-zwemwater is meer te lezen over wat de gevolgen zijn van blauwalgen voor de zwemwaterkwaliteit.

::: aside
![Veel blauwalgen in het water](images/blauwalgen.jpg){group="toestand"}
:::

{{< pagebreak >}}

```{r}
#| label: fig-kaart-blauwalgen
#| out-width: "100%" 
#| fig-height: 3.5
#| fig-cap: "Op welke locaties waren er veel blauwalgen? De kaart is gemaakt op basis van de grootst gemeten hoeveelheid blauwalgen in {{< var rap_jaar >}}. De indeling is gemaakt op basis van de risicobeoordeling voor zwemwater. (interactief)"
#| fig-alt: "Kaart met de locaties waarin {{< var rap_jaar >}} veel blauwalgen zijn aangetroffen."
kaart_blauwalgen
```
